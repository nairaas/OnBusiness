...
static NSObject<BTLocationTextFieldDataSource> *DefaultAutocompleteDataSource = nil;

@interface BTLocationTextField ()
...
@implementation BTLocationTextField
...
- (void)refreshAutocompleteText
{
    if (!self.autocompleteDisabled)
    {
        id <BTLocationTextFieldDataSource> dataSource = nil;
        
        if ([self.autocompleteDataSource respondsToSelector:@selector(textField:completionForPrefix:ignoreCase:)])
        {
            dataSource = (id <BTLocationTextFieldDataSource>)self.autocompleteDataSource;
        }
